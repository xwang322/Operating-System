...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
      bfree(ip->dev, (ip->addrs[i] & 0x00ffffff)); // we only saves 24 bits as done in bmap, so only needs to clear 24 bits
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, (ip->addrs[NDIRECT] & 0x00ffffff));
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
      if(a[j])
        bfree(ip->dev, (a[j] & 0x00ffffff));
    }
    brelse(bp);
    bfree(ip->dev, (ip->addrs[NDIRECT] & 0x00ffffff));
    ip->addrs[NDIRECT] = 0;
  }
...
stati(struct inode *ip, struct stat *st)
{
  ...
  uint *a;
  struct buf *bp;
  uint tmp = 0, checksum_tmp = 0, checksum = 0;
  ...
  if (ip->type == T_CHECKED){
    for (i = 0; i < (NDIRECT+1); i++){
      checksum_tmp = 0;
      if (i < NDIRECT){ // i is directing to direct pointer
        tmp = (ip-> addrs[i] >> 24);  // data type has to match, takes a very long long time to debug this one, char cannot shift 24 bits
        checksum_tmp = tmp;
        checksum ^= checksum_tmp;
      }
      else{  // i in directing to the indirect pointer
        bp = bread(ip->dev, ip->addrs[NDIRECT]);  // get the address of the indirect pointer
        a = (uint*)bp->data;  // data there is second level pointers, maximum 512/4 = 128 
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  int i = 0, readload = 0; 
  uchar tmp = 0;
  uint checksum = 0, blocknum = 0, checksum2 = 0;
  uint* a = NULL;
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    if (ip->type == T_CHECKED){
      blocknum = off / BSIZE;
      if (blocknum < NDIRECT){
        checksum = ip->addrs[blocknum] & 0xff000000;
      ...
      if (blocknum < NINDIRECT){
        ...
        a = (uint*)bp2->data;
        checksum = a[blocknum] & 0xff000000;
        checksum = checksum >> 24;
        brelse(bp2);
      }
      
      tmp = 0; // very imp, remember to set back to zero every time
      for (i = 0; i < readload; i++){
        tmp ^= bp->data[i];
      }
      checksum2 = (uint) tmp;
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    tmp = 0; // very imp, remember to set back to zero every time
    if (ip->type == T_CHECKED){
      for (i = 0; i < writeload; i++){
        tmp ^= bp->data[i];
      }
      checksum = (uint)tmp;
      checksum = checksum << 24;

      blocknum = off / BSIZE; // number of blocks need writing
      if (blocknum < NDIRECT){
        ip->addrs[blocknum] &= 0x00ffffff;
        ip->addrs[blocknum] |= checksum;
      }
      blocknum -= NDIRECT;
