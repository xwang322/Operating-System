...
// main function
   ...
   // batch mode
   // I/O value for different function
   // history
...
main (int argc, char *argv[]){
   ...
   // Tell whether it is a valid input command
    while (fgets(input, in_size, stream)){   // this is only for a real file, not for keyboard input
   ...
   //     if (compare(input, void_input) == 1){ //strcmp cannot compare a null string with another null string   /// still have confusion
	...
	if(input[strlen(input) - 1] == '\n'){ 
        ...
        int fd_out = 0;
        if (is_redir){
		close(STDOUT_FILENO); // change from Jason
