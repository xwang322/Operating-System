...
void requestHandle(struct requestStat *request)
{
   ...
   if (strcasecmp(method, "GET")) {
      ...
      request->error_code = 501;
      return;
   }
   requestReadhdrs(&rio);

   request->is_static = requestParseURI(uri, request->filename, cgiargs);
   if (stat(request->filename, &sbuf) < 0) {
      requestError(fd, request->filename, "404", "Not found", "CS537 Server could not find this file");
   ...
   if (request->is_static) {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
         requestError(fd, request->filename, "403", "Forbidden", "CS537 Server could not read this file");
         request->error_code = 403;
         return;
      }
      requestServeStatic(fd, request->filename, sbuf.st_size);
   }  
   else {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
         requestError(fd, request->filename, "403", "Forbidden", "CS537 Server could not run this CGI program");
         request->error_code = 403;
         return;
      }
      requestServeDynamic(fd, request->filename, cgiargs);
   }
}
