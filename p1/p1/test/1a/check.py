...
def check_sorted(sorted_file, orig_file, lowValue, highValue):
   count = Counter()
   ...
   with open(orig_file, "rb") as f:
      while True:
         ...
         orig_key = get_key(rec)

         if lowValue <= orig_key and orig_key <= highValue:
            count[rec] += 1
...
def main():
   ...
   if len(sys.argv) != 5:
      ...
      sys.exit(2)
   (is_sorted, msg) = check_sorted(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
   print msg
   if is_sorted:
      sys.exit(0)
   else:
